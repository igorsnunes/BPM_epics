...
static int get_ao(int sock, epicsUInt8 **buf, int instrument_id, int variables){
	command_header ask,answer;
	int  size, ret_val;
	size = var_write_command_ask(&ask,4);
	ask.p[0] = (unsigned char)variables;
	ask.p[1] = (unsigned char)(*buf)[0];
	ask.p[2] = (unsigned char)(*buf)[1];
	ask.p[3] = (unsigned char)(*buf)[2];
	ask.p[4] = (unsigned char)(*buf)[3];
	ret_val = comm_talk(sock,ask,size,&answer,instrument_id);
	return ret_val;
}
...
static int get_operation(int sock, epicsUInt8 **buf, int instrument_id, int variable,enum operation op){
	...
	switch(op){
		...
		case OP_WRITE_BO:
			...
			break;
		case OP_WRITE_AO:
			return get_ao(sock,buf,instrument_id,variable);	
			break;
	}
	return 0;
}
